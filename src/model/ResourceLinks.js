...
export class ResourceLinks {
  ...
  async indexResources() {
    try {
      this.resources = await this.db("resources").select();
    } catch (e) {
      console.error(e);
    }
  ...
  async getResource(id) {
    return await this.db("resources").where("id", id).select().first();
  }
  ...
  async createResource(name, href) {
    return await this.db("resources").insert({ name, href });
  }

  async mapResourceToTemplateTask(resourceId, templateId, taskId) {
    return await this.db("templateResources").insert({
      resourceId,
      templateId,
      taskId,
    });
  ...
  async getTemplateTaskResources(templateId, taskId) {
    return await this.db("templateResources")
      ...
      .select(
        "resources.*",
        "templateResources.templateId",
        "templateResources.taskId",
        "templateResources.resourceId"
      );
  }
  ...
  async getTemplateResources(templateId) {
    return await this.db("templateResources")
      ...
      .select(
        ...
        "templateResources.templateId",
        "templateResources.taskId",
        "templateResources.id",
        "templateResources.resourceId"
      );
  }

  async deleteTemplateResources(templateId) {
    return await this.db("templateResources").where({ templateId }).del();
  }

  async mapResourceToProjectTask(resourceId, projectId, taskId) {
    return await this.db("projectResources").insert({
      resourceId,
      projectId,
      taskId,
    });
  ...
  async deleteProjectResourceMapping(resourceId) {
    return await this.db("projectResources").where("id", resourceId).del();
  }

  async deleteTemplateResourceMapping(resourceId) {
    return await this.db("templateResources").where("id", resourceId).del();
  }

  async deleteResource(id) {
    await this.db("projectResources").where({ resourceId: id }).del();

    await this.db("templateResources").where({ resourceId: id }).del();

    return await this.db("resources").where("id", id).del();
  }

  async updateResource(id, name, href) {
    return await this.db("resources").where("id", id).update({ name, href });
  }
}
